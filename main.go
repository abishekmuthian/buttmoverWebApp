...
// If custom font is needed, Right now I'm using the fonts from ebitenengine directly
/*func init() {
...
// NewGame initializes the game object and calls the init functions
func NewGame() *Game {
	g := &Game{}
	g.init()
	return g
...
// Layout sets the Game dimensions
func (g *Game) Layout(outsideWidth, outsideHeight int) (screenWidth, screenHeight int) {
	return 640, 480
}

// setErrorStatus sets the error from JS
...
// setConnectionStatus sets the connection status from JS
...
// setColorMode sets the color mode (dark/light) from JS
...
// setClock sets the clock from JS
func setClock(this js.Value, args []js.Value) interface{} {
	clockStarted = true
	return nil
}

// setScore sets the score when called from JS
...
// setButtonTrigger toggles the butt trigger status
...
// setTick sets the time when called from JS
...
// FIXME: Attempted to write the timers within Go, instead of in JS but results in race conditions and hangs up.
// Possible bug within Go Web Assembly? Should Isolate, Test and report to Go issues.
/*func tick25() js.Func {
...
// drawTiles draws the tiles on the screen
...
// drawBulldozer draws the bulldozer on the screen
...
// drawChair draws the chair on the screen
...
// drawButt draws the butt on the screen
